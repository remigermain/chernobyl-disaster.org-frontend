<template>
  <admin-field-base :field="field" :errors="errors">
    <div class="flex flex-wrap flex-col space-y-2">
      <div
        class="preview-image flex items-center justify-center"
      ...
      >
        <div v-if="imageSrc" class="relative h-full w-full">
          ...
          />
          <span
            class="w-full h-full absolute top-0 left-0 cursor-pointer duration-300 transition-opacity justify-center opacity-0
                      hover:opacity-100 items-center flex z-10 picture-hover"
            @click="active = true"
          >
      ...
      <label
        class="p-2 rounded-full bg-indigo-700 h-10 text-gray-300 space-x-2 group cursor-pointer hover:bg-indigo-600 flex justify-center items-center"
      >
        <svg-icon
          name="upload"
          class="h-full group-hover:scale-125 transform transition-transform duration-300"
        />
        <span v-if="original">
