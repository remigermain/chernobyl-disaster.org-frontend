  ...
  <admin-field-base :field="field" :errors="errors">
    <div class="flex flex-wrap flex-col space-y-2">
      <vue-datetime v-model="valueModel.date" class="datetime" :required="field.required" :phrases="{ok: 'OK', cancel: $t('word.cancel')}">
        ...
        <svg-icon slot="after" name="x" class="h-full opacity-50 text-2xl cursor-pointer hover:scale-110 hover:text-indigo-700" @click="valueModel.date = null" />
      </vue-datetime>
      <div class=" flex justify-around shadow-lg text-gray-800 dark:bg-gray-600 dark:text-gray-200 p-2 rounded-sm items-center text-2xl" :class="{'disabled': disabled}">
      ...
      <label class="grid-time shadow-md text-gray-300 rounded-sm dark:bg-gray-600" :class="{'disabled': disabled}">
        ...
        <span class="flex justify-center items-center p-2">
          <input v-model="valueModel.hours" type="range" min="00" max="23" :disabled="disabled"/>
        </span>
      </label>
      <label class="grid-time shadow-md text-gray-300 rounded-sm dark:bg-gray-600" :class="{'disabled': disabled}">
        <div class="h-full h-full flex justify-center items-center rounded-sm p-2" :class="{'bg-gray-900': !disabled}">
        ...
        <span class="flex justify-center items-center p-2">
          <input v-model="valueModel.minutes" type="range" min="00" max="59" :disabled="disabled"/>
        </span>
      </label>
      <label class="grid-time shadow-md text-gray-300 rounded-sm dark:bg-gray-600" :class="{'disabled': disabled}" >
        <div class="h-full h-full flex justify-center items-center rounded-sm p-2" :class="{'bg-gray-900': !disabled}">
        ...
        <span class="flex justify-center items-center p-2">
          <input v-model="valueModel.seconds" type="range" min="00" max="59" :disabled="disabled"/>
        </span>
      </label>
    </div>
  </admin-field-base>
