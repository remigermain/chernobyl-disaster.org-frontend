<template>
  <div class="w-full">
    <div
      class="w-full border-gray-500 dark:bg-gray-800 border-t-4 rounded-t-md p-2 mt-2 cursor-pointer italic text-opacity-75"
      :class="{ 'border-indog-700': active, 'rounded-b-md': !active }"
      @click="active = !active"
    >
      <svg-icon
        name="arrow-down"
        class="transform transition-transform duration-400"
    ...
    <div class="relative overflow-hidden dark:bg-gray-800 rounded-b-md">
      <div
        class="w-full flex space-y-1 leading-6 transform p-2 transition-all duration-400 justify-around"
        :class="{
          'scale-y-0 absolute opacity-0': !active,
          'scale-y-100 opacity-100 block': active
        }"
      >
        <div
          class="rounded-md border shadow-md p-2 dark:bg-gray-900 dark:border-none"
        >
          ...
          <form class="flex flex-wrap p-3 " @submit.prevent="submit">
            <div class="flex flex-col">
              <field-select
                ...
                :field="modelField.language"
                :errors="errors.language"
              />
              <field-checkbox
                v-model="data.delete"
                :field="modelField.delete"
                :errors="errors.delete"
              />
              <field-checkbox
                v-model="data.nolang"
                :field="modelField.nolang"
                :errors="errors.nolang"
              />
              <field-checkbox
                v-model="data.merge"
                :field="modelField.merge"
                :errors="errors.merge"
              />
              <field-checkbox
                v-model="data.parent"
                :field="modelField.parent"
                :errors="errors.parent"
              />
            </div>
            <field-file
              v-model="data.file"
              :field="modelField.file"
              :errors="errors.file"
              class="w-2/4"
            />
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$axios
        .post('populate/translate/upload', form)
        .then(response => {
