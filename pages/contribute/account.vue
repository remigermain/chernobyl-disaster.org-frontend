<template>
  <div class="flex flex-col space-y-2">
    ...
    <div
      class="flex justify-around flex-wrap account shadow-md border rounded-md dark:bg-gray-800 dark:border-none"
    >
        ...
        <div class="-sm:w-full py-4 rounded password">
          ...
          <form class="p-4 rounded space-y-2" @submit.prevent="submitSettings">
            <field-checkbox
              ...
              v-model="data.show_admin"
              :field="modelField.show_admin"
              :errors="errors.show_admin"
            />
            <field-checkbox
              v-model="data.show_helpers"
              :field="modelField.show_helpers"
              :errors="errors.show_helpers"
            />
            <div class="flex justify-end w-full">
              <field-submit class="w-min-content">
                {{ $t('word.update') }}
    ...
    <div
      class="flex justify-start flex-col account shadow-md border rounded-md dark:border-none"
    >
      <div class="rounded">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 dark:bg-gray-800">
          <div class="flex items-center">
            <div
              class="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 hover:bg-red-200 transition-colors duration-300
                        cursor-pointer group"
              @click="active = true"
            >
              <svg-icon
                name="x"
                class="h-6 w-6 text-red-600 group-hover:scale-125 transform duration-300 transition-transform"
              />
            </div>
            <div class="ml-3 text-center">
              <h3
                class="text-lg leading-6 font-medium text-gray-900 text-opacity-75 dark:text-gray-400"
              >
    ...
    <utils-modal v-if="active" @close="active = false">
      <template #content>
        ...
        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
          <h3
            class="text-lg leading-6 font-medium text-gray-900dark:text-gray-400 "
          >
...
export default {
  ...
  created() {
    this.field.old_password = {
      ...this.modelField.password,
      ...this.field.old_password
    }
    this.field.new_password1 = {
      ...this.modelField.password,
      ...this.field.new_password1
    }
    this.field.new_password2 = {
      ...this.modelField.password,
      ...this.field.new_password2
    }
  },
      ...
      this.$axios
        ...
        .catch(error => {
          this.responseError(error).then(data => {
            ...
            data.new_password1 &&
              (this.errors.new_password1 = data.new_password1)
            data.new_password2 &&
      ...
      */
      ...
      this.$axios
        ...
        .then(response => {
          ...
          this.$toast.success(this.$t('success-message.update'))
          this.$auth.fetchUser()
        })
        .catch(error => {
          this.responseError(error).then(data => {
    ...
    submitDelete() {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          this.$toast.success(this.$t('success-message.account-delete'))
          this.$auth.logout()
