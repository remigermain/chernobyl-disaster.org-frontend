<template>
  <div class="flex flex-wrap justify-center p-4 gap-4 space-y-2">
    <div class="w-full space-y-2">
      <admin-utils-header :title="model.name" :description="$t('description.event')">
      ...
      <admin-utils-report v-if="activeModalReport" :id="object.id" uuid="event" @close="activeModalReport = false" />
...
export default {
  ...
  head () {
    const title = `${this.$t("menu.event")} - ${this.$t("word.detail")}`
    return {
      title,
      meta: [
