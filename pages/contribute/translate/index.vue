          ...
          <template #tbody>
            <tr v-for="(obj, idx) in object" :key="idx">
              ...
              <td class="text-center">
                <span
                  class="text-white dark:text-gray-300 rounded-lg w-full block bg-gradient-to-r"
                  :class="{
                    'from-black via-black to-red-700': obj.ratio < 1,
                    'from-black via-red-700 to-red-700':
                      obj.ratio >= 1 && obj.ratio <= 20,
                    'from-red-700 via-orange-700 to-orange-700':
                      obj.ratio > 20 && obj.ratio <= 40,
                    'from-orange-700 via-yellow-700 to-yellow-700':
                      obj.ratio > 40 && obj.ratio <= 60,
                    'from-yellow-700 via-blue-700 to-blue-700':
                      obj.ratio > 60 && obj.ratio <= 80,
                    'from-blue-700 via-green-700 to-green-700':
                      obj.ratio > 80 && obj.ratio <= 99,
...
export default {
  ...
  asyncData({ app, store }) {
    return app.$axios
      .get('populate/translate/overview')
      .then(response => {
  ...
  methods: {
    ...
    pathEdit(obj) {
      return {
        name: `contribute-translate-edit-id`,
        params: { id: obj.language }
      }
    }
  }
}
