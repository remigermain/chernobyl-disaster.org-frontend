    ...
    <div class="w-full space-y-2">
      ...
      <div class="w-full space-y-4">
        ...
        >
          ...
          <template #tbody>
            <tr v-for="(obj, idx) in object" :key="idx">
              ...
              <td v-else class="text-gray-500 italic text-sm">
                {{ $t('word.empty') }}
              </td>
              <td class="space-x-1">
        ...
        <lazy-admin-utils-table
          ...
          @pagination="setLangPagination"
        >
          <template #header>
            <admin-utils-table-header
              v-model="searchValueLang"
              :title="$t('text.need-translation')"
              @search="refreshLang"
            />
          </template>
          <template #thead>
          ...
          <template #tbody>
            <tr v-for="(obj, idx) in objectLang" :key="idx">
              ...
              <td>{{ obj.title }}</td>
              <td v-if="obj.event">{{ obj.event }}</td>
              <td v-else class="text-gray-700 italic text-sm">
                {{ $t('word.empty') }}
              </td>
  ...
  async asyncData({ redirect, $axios, app, store }) {
    try {
      ...
      const responseLang = await $axios.get(
        generateUrl('picture', 1, null, false)
      )
