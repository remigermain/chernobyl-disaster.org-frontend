        ...
        <template #head>
          <div class="flex flex-col justify-center space-y-4 text-center">
            ...
            <div class="p-2 relative preview-image">
              <picture>
                <source
                  :srcset="object.picture.thumbnail_webp"
                  media="(max-width: 550px)"
                  type="image/webp"
                />
                <source
                  :srcset="object.picture.thumbnail_jpeg"
                  media="(max-width: 550px)"
                  type="image/jpeg"
                />
                <source
                  :srcset="object.picture.original_webp"
                  type="image/webp"
                />
...
export default {
    ...
    return app.$axios
      ...
      .then(response => {
        ...
        response.data.picture &&
          convertImageUrl(response.data.picture, app.$media)
  ...
  head() {
    const title = `${this.$t('menu-name.picture')} - ${this.$t('word.detail')}`
    return {
      title,
      meta: [
