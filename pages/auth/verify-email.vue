<template>
  ...
  >
    ...
    <form
      class="my-4 w-3/4 min-min-w-max-content form"
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$axios
        .post('auth/registration/verify-email/', { key: this.$route.query.key })
        .then(response => {
          if (response.status !== 200) {
            throw new Error('error-server')
          }
          this.$toast.success(this.$t('success-message.email-verified'))
